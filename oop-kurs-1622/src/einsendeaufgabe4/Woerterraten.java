...
public class Woerterraten {
	...
	void aktualisiereLoesung(char c) {
		System.out.println("Super, der Buchstabe " + c + " ist enthalten.");
		int counter = 0;
		while (counter < zuRatendesWort.length()) {
			char currentChar = zuRatendesWort.charAt(counter);
			boolean gleich = (currentChar == c);
			if (gleich == true) {
				erratenesWort.setCharAt(counter, c);
			}
			counter++;
		}
	}
